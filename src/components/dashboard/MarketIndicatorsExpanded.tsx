...
const MarketIndicatorsExpanded = () => {
  ...
  // 6 top market cap stocks
  const topStocks = [
    { symbol: 'AAPL', displayName: 'Apple', delay: 0.3 },
    { symbol: 'MSFT', displayName: 'Microsoft', delay: 0.35 },
    { symbol: 'GOOGL', displayName: 'Alphabet', delay: 0.4 },
    { symbol: 'AMZN', displayName: 'Amazon', delay: 0.45 },
    { symbol: 'NVDA', displayName: 'NVIDIA', delay: 0.5 },
    { symbol: 'META', displayName: 'Meta', delay: 0.55 },
  ];
    ...
    <div className="space-y-6">
      ...
      >
        ...
        {isSearchMode && (
          <button
            onClick={clearSearch}
            className="flex items-center whitespace-nowrap ml-2 gap-2 px-3 py-1.5 text-xs font-medium bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors"
          >
            <X className="w-3 h-3" />
      ...
      {/* Grid - 12 items (6 indices + 6 stocks) or search result */}
        ...
        {isSearchMode && searchedTicker ? (
          ...
          <>
            <MarketCard
              ...
              delay={0}
              onClick={() => handleTickerClick(searchedTicker, searchedTicker)}
            />
          ...
          // Show 6 indices + 6 top stocks
          <>
            {indices.map((indicator, idx) => (
              <MarketCard
            ...
            {topStocks.map((stock, idx) => (
              <MarketCard
                ...
                delay={stock.delay}
                onClick={() =>
                  handleTickerClick(stock.symbol, stock.displayName)
                }
              />
            ))}
          </>
...
export default MarketIndicatorsExpanded;
