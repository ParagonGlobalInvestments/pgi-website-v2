...
export default function MarketWatchNews() {
  ...
  const fetchNewsItems = async (isRefresh = false) => {
    try {
      ...
      const response = await fetch('/api/rss-items?source=marketwatch-top');
  ...
  useEffect(() => {
    ...
    registerRefreshCallback('marketwatch', refreshCallback);

    return () => {
      unregisterRefreshCallback('marketwatch');
    };
  ...
  const handleServerRefresh = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/rss-items/refresh?role=${userRole}`, {
        method: 'POST',
      });
