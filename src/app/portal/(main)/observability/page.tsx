...
const VITALS_CONFIG: Record<string, {
  good: number;
  ...
  return (
    <Card className="border-gray-200">
      <CardContent className="pt-6">
        ...
        {trend && (
          <div className={`mt-2 text-xs flex items-center ${trend === 'up' ? 'text-green-500' : 'text-red-500'}`}>
...
function VitalCard({ metric, data }: { metric: string; data: {
  avg_value: number;
  p75_value: number;
  good_count: number;
  ...
  const goodPct = total > 0 ? ((data?.good_count ?? 0) / total * 100).toFixed(0) : '0';
  const needsPct = total > 0 ? ((data?.needs_improvement_count ?? 0) / total * 100).toFixed(0) : '0';
  const poorPct = total > 0 ? ((data?.poor_count ?? 0) / total * 100).toFixed(0) : '0';
    ...
    <Card className="border-gray-200 overflow-hidden">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full ${getStatusBg(p75, metric)} ${getStatusColor(p75, metric)}`}>
            {p75 <= config.good ? 'Good' : p75 <= config.needsImprovement ? 'Needs Work' : 'Poor'}
          </span>
        </div>
      </CardHeader>
      <CardContent>
        ...
        {total > 0 && (
          <>
            <div className="flex gap-0.5 h-2 rounded-full overflow-hidden mt-3">
              ...
              <div className="bg-yellow-500 transition-all" style={{ width: `${needsPct}%` }} />
...
function DeviceBreakdown({ data }: { data: { desktop: number; mobile: number; tablet: number; unknown: number } }) {
  const total = data.desktop + data.mobile + data.tablet + data.unknown;
  if (total === 0) return <p className="text-sm text-gray-400">No device data</p>;

  const items = [
    { key: 'desktop', label: 'Desktop', icon: Monitor, value: data.desktop, color: 'bg-blue-500' },
    { key: 'mobile', label: 'Mobile', icon: Smartphone, value: data.mobile, color: 'bg-green-500' },
    { key: 'tablet', label: 'Tablet', icon: Tablet, value: data.tablet, color: 'bg-purple-500' },
    ...
    <Card className={`${config.border} ${config.bg}`}>
      <CardContent className="pt-6">
        <div className="flex items-center gap-4">
          <div className={`h-14 w-14 rounded-full ${config.bg} flex items-center justify-center`}>
...
export default function AnalyticsPage() {
  ...
  const vitalsMap = useMemo(() => {
    ...
    return data.vitals.summary.reduce((acc: Record<string, unknown>, item: { metric_type: string }) => {
  ...
  const trendsByMetric = useMemo(() => {
    ...
    data.vitals.trends.forEach((t: { metric_type: string; p75_value: number }) => {
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">See how the public site is performing</p>
        </div>
        <div className="flex items-center gap-2">
          <select
            value={days}
        ...
        <>
          ...
          {data?.traffic?.totalPageviews === 0 && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-blue-800">Analytics is ready</p>
                <p className="text-sm text-blue-600 mt-0.5">
                  Data will appear automatically as visitors browse the site. This page updates every minute.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <SiteHealthSummary vitalsMap={vitalsMap} errorCount={data?.errors?.total || 0} />
            <StatCard
              title="Page Views"
            ...
            <CardContent>
              {data?.traffic?.dailyTrends?.length > 0 ? (
                <div className="flex items-end gap-1 h-24">
                  {data.traffic.dailyTrends.map((day: { day: string; count: number }) => {
                    const maxCount = Math.max(...data.traffic.dailyTrends.map((d: { count: number }) => d.count));
                ...
                <div className="py-6 text-center">
                  ...
                  <p className="text-sm text-gray-500">Traffic data will appear here</p>
                  <p className="text-xs text-gray-400 mt-1">Data is automatically collected from site visitors</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-gray-200">
            ...
            <CardContent>
              {data?.traffic?.topPages?.length > 0 ? (
                <div className="space-y-2">
                  {data.traffic.topPages.slice(0, 5).map((page: { path: string; pageviews: number }, i: number) => (
                    <div key={page.path} className="flex items-center gap-3 py-1.5 border-b border-gray-50 last:border-0">
                      <span className="text-xs text-gray-400 w-5">{i + 1}</span>
                      <span className="flex-1 text-sm text-gray-700 truncate">{page.path === '/' ? 'Home' : page.path}</span>
                      <span className="text-sm font-medium text-gray-600">{page.pageviews} views</span>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="py-6 text-center">
                  ...
                  <p className="text-sm text-gray-500">Page rankings will appear here</p>
                  <p className="text-xs text-gray-400 mt-1">Shows which pages get the most visits</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <CollapsibleSection title="Performance Details" icon={Activity} defaultOpen={false}>
            {/* Core Web Vitals */}
            <div className="mb-6">
              <h3 className="text-sm font-medium text-gray-600 mb-3">Core Web Vitals</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                {['LCP', 'FCP', 'CLS', 'TTFB', 'INP'].map(metric => (
                  <VitalCard key={metric} metric={metric} data={vitalsMap[metric] || null} />
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <h3 className="text-sm font-medium text-gray-600 mb-3">Performance Trends</h3>
                <div className="space-y-4 bg-gray-50 rounded-lg p-4">
                  {['LCP', 'FCP', 'TTFB'].map(metric => (
                    <div key={metric} className="flex items-center gap-4">
                      <span className="w-12 text-sm font-medium text-gray-600">{metric}</span>
              ...
              <div>
                <h3 className="text-sm font-medium text-gray-600 mb-3">Device Breakdown</h3>
                <div className="bg-gray-50 rounded-lg p-4">
                  <DeviceBreakdown data={data?.traffic?.deviceBreakdown || { desktop: 0, mobile: 0, tablet: 0, unknown: 0 }} />
                </div>
              </div>
            </div>
          </CollapsibleSection>
          ...
          >
            {data?.errors?.recent?.length > 0 ? (
              <div className="space-y-3 max-h-[400px] overflow-y-auto">
                {data.errors.recent.map((error: { id: string; error_type: string; message: string; path: string; created_at: string }) => (
                  <div key={error.id} className="p-3 bg-red-50/50 rounded-lg border border-red-100">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-red-600 uppercase">{error.error_type}</span>
                      <span className="text-xs text-gray-400">
                        {new Date(error.created_at).toLocaleString()}
                      </span>
                    </div>
                    <p className="text-sm text-gray-700 mt-1 line-clamp-2">{error.message}</p>
                    <p className="text-xs text-gray-400 mt-1 font-mono">{error.path}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-gray-400 mt-1">Your site is running smoothly</p>
              </div>
            )}
          </CollapsibleSection>

          {/* Extended Top Pages - Collapsed */}
          <CollapsibleSection title="Detailed Page Analytics" icon={BarChart3} defaultOpen={false}>
            {data?.traffic?.topPages?.length > 0 ? (
              <div className="space-y-2">
                {data.traffic.topPages.map((page: { path: string; pageviews: number; avg_lcp: number | null }, i: number) => (
                  <div key={page.path} className="flex items-center gap-3 py-2 border-b border-gray-100 last:border-0">
                    <span className="text-xs text-gray-400 w-6">{i + 1}</span>
                    <span className="flex-1 text-sm text-gray-700 truncate font-mono">{page.path}</span>
                    <span className="text-sm text-gray-500 w-20 text-right">{page.pageviews} views</span>
                    {page.avg_lcp && (
                      <span className={`text-xs w-16 text-right ${getStatusColor(page.avg_lcp, 'LCP')}`}>
